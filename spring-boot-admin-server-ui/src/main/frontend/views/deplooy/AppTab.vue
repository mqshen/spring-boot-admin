<template>
  <div class="appTabDiv">
    ...
    <a-table
      ...
      :loading="loading"
      :rowKey="(record) => record.instances ? record.id : record.sbaId">
      <span slot="status" slot-scope="text, record">
        ...
        <template v-else-if="record.instances">
          {{ record.instances | statusFilter | finalStatus }}
        </template>
        <template v-else>
    ...
    <app-modal ref="modal" @ok="handleOk" />
    <build-log ref="logModal" />
    <app-set-modal ref="setModal" @ok="handleSOk" :servers="servers" :groups="groups" />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    handleOk () {
      ...
      window.console.log('新增成功后重新拿全部数据？？')
    },
    handleSOk () {
      this.$message.success('设置成功')
      window.console.log('设置成功后改行状态如何变化？？')
    },
    addEvent () {
      this.$refs.modal.add()
    },
    ...
    filterApplications () {
      ...
      if (this.queryParam.eve && this.queryParam.eve > 0) {
        mathServer = this.servers.filter((server) => {
      ...
      const applications = this.allApplications.filter((application) => {
        ...
        const instances = application.children.map((id) => {
          ...
          if (this.queryParam.eve && this.queryParam.eve > 0) {
            const found = mathServer.filter((server) => {
              window.console.log(instance.serverId)
              return server.id === instance.serverId
            }).length
            if (found === 0) {
              return false
            }
