...
import { doAddDeployInstance } from '@/api/deploy.js'
  ...
  methods: {
    handleSubmit () {
      this.form.validateFields((err, values) => {
        if (!err) {
          doAddDeployInstance(values).then(() => {
            this.visible = false
            this.$emit('handleSOk')
          })
    ...
    add (record) {
      ...
      _this.$nextTick(() => {
        setTimeout(() => {
          _this.form.setFieldsValue(pick(_this.mdl, 'serviceId'))
        })
      })
    },
    edit (record) {
      ...
      _this.$nextTick(() => {
        setTimeout(() => {
          _this.form.setFieldsValue(pick(_this.mdl, 'id', 'serviceId', 'serverId',
